...
class Kinect(threading.Thread):
    def __init__(self, color=False,
                       ...
                       undistorted=False,
                       pointcloud=True,
                       remove_black=False,  # Remove all (0.0, 0.0, 0.0) colors
                       debug=False):
    ...
    def compute_pointcloud(self, color, depth):
        ...
        if self._scale_rgb:
            point_cloud[:,3:] = valid_rgb / 255.0
        else:
            point_cloud[:,3:] = valid_rgb

        if self.remove_black:
            point_cloud = np.delete(point_cloud, np.where(point_cloud[:,3:]==(0.0, 0.0, 0.0)), axis=0)
